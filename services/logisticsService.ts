...
export class LogisticsService extends BaseService {
    ...
    async getDriverJobs(driverId: string): Promise<ParcelBooking[]> {
        // Driver needs:
        // 1. Jobs where they are involved (collector, current driver, or deliverer)
        // 2. Available jobs (status == 'PENDING' for pickup)

        try {
            // Query 1: Involved in any capacity (uses denormalized index)
            const q1 = query(collection(this.db, 'parcel_bookings'), where('involvedDriverIds', 'array-contains', driverId));
            const snap1 = await getDocs(q1);

            // Query 2: Pending jobs (Available for pickup - no driver assigned yet)
    ...
    async saveParcelBooking(b: ParcelBooking, paymentAccountId?: string, walletService?: any, configService?: any) {
        ...
        // --- AUTO-POPULATE involvedDriverIds for query visibility ---
        const involvedDriverIds = new Set<string>();
        if (bookingToSave.driverId) involvedDriverIds.add(bookingToSave.driverId);
        (bookingToSave.items || []).forEach((item: any) => {
            if (item.driverId) involvedDriverIds.add(item.driverId);
            if (item.collectorId) involvedDriverIds.add(item.collectorId);
            if (item.delivererId) involvedDriverIds.add(item.delivererId);
        });
        bookingToSave.involvedDriverIds = Array.from(involvedDriverIds);
        // --- END AUTO-POPULATE ---
