...
export const AccountList: React.FC<Props> = ({ accounts, transactions = [], onEdit, onDelete }) => {
  ...
  const handleVerifyDelete = (account: Account) => {
      ...
      if (hasChildren) {
          alert("Cannot delete this account because it has sub-accounts attached. Please delete or re-parent the sub-accounts first.");
          return;
      }
                        ...
                        <tr key={acc.id} className={`hover:bg-gray-50 ${acc.isHeader ? 'bg-gray-50/80' : ''}`}>
                            ...
                            <td className="px-4 py-2 text-sm align-middle">
                                <span className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full border ${
                                    acc.type === 'Asset' ? 'bg-green-50 text-green-700 border-green-200' :
