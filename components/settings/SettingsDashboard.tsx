...
export const SettingsDashboard: React.FC<Props> = ({
  ...
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState<'GENERAL' | 'COA' | 'BRANCHES' | 'CURRENCIES' | 'TAXES' | 'COMMISSIONS' | 'REFERRAL' | 'MENU' | 'RULES'>('GENERAL');
          ...
          try {
              ...
              if (newAccounts.length > 0) {
                  if (confirm(`Ready to import ${newAccounts.length} accounts? Existing accounts with same Code will be updated.`)) {
                      ...
                      alert(`Successfully processed ${newAccounts.length} accounts.`);
                  }
              } else {
                  alert("No valid account rows found in CSV.");
              }
  ...
  const handleLoadMaster = async () => {
      ...
      if (confirm(`This will merge ${MASTER_COA_DATA.length} accounts from the Master COA into your database. Existing accounts with matching codes will be updated. Continue?`)) {
          ...
          try {
              ...
              alert("Master COA loaded successfully.");
          } catch (e) {
              console.error(e);
              alert("Failed to load master COA.");
          } finally {
              setImporting(false);
          }
      }
