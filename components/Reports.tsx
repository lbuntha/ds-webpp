...
import { DelayedChatReport } from './reports/DelayedChatReport';
...
export const Reports: React.FC<Props> = ({ transactions, accounts, branches }) => {
    const { t } = useLanguage();
    const [activeReport, setActiveReport] = useState<ReportType | 'WALLET' | 'TRANSIT' | 'DELAYED'>('TB');
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center bg-white p-2 rounded-xl shadow-sm border border-gray-100">
                    <div className="flex space-x-2 overflow-x-auto">
                        ...
                        <div className="h-8 w-px bg-gray-300 mx-2 self-center"></div>
                        <span className="px-4 py-2 rounded-lg text-sm font-bold text-red-800 bg-red-50">Delayed Chats</span>
                    </div>
                </div>
                <DelayedChatReport />
            </div>
                        ...
                        <div className="flex space-x-2 overflow-x-auto pb-2">
                            {(['TB', 'BS', 'IS', 'GL'] as ReportType[]).map((type) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${activeReport === type
                            ...
                            <button
                                onClick={() => setActiveReport('DELAYED')}
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap text-gray-600 hover:bg-gray-50`}
                            >
                                Delayed Chats
                            </button>
