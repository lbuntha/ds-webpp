...
export const OnboardingWizard: React.FC<Props> = ({ onComplete }) => {
  ...
  const handleNext = () => {
    if (step === 1 && !companyName) return alert('Please enter a company name.');
    if (step === 2 && (!branchName || !branchCode)) return alert('Please define at least one branch.');
        ...
        <div key={i} className="flex items-center">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${
              step >= i ? 'bg-indigo-600 text-white shadow-md scale-110' : 'bg-gray-200 text-gray-500'
            }`}
          >
            {i}
          </div>
