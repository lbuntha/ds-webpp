        ...
        // 2. Handle Parcels (Driver Settlement)
            ...
            for (const rel of transaction.relatedItems) {
                ...
                if (booking) {
                    ...
                    if (parcelItem) {
                        // Estimate comm per item
                        const totalBookingFee = booking.totalDeliveryFee;
                        const itemsCount = bItems.length || 1;
                        let itemComm = 0;
                        
                        // Simple Logic: Distribute fee evenly
                        if (defaultRule.type === 'FIXED_AMOUNT') {
                            itemComm = defaultRule.value / itemsCount;
                        } else {
                            // Percentage
                            itemComm = (totalBookingFee / itemsCount) * (defaultRule.value / 100);
                        }
                                ...
                                {items.length > 0 && (
                                    <tr className="bg-gray-100 font-bold border-t-2 border-gray-200">
                                        ...
                                        {transaction.type === 'SETTLEMENT' && items.some(i => i.commission) && (
                                            <td className="px-3 py-2 text-right text-green-700">{totalCommission.toFixed(2)} USD</td>
                                        )}
                                    </tr>
                                )}
                            </tbody>
