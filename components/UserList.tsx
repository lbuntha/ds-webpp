...
export const UserList: React.FC<Props> = ({ users, branches = [], rolePermissions, onUpdateRole, onUpdateStatus, onUpdateProfile, onUpdatePermissions, onSyncProfile }) => {
  ...
  const handleSyncCustomer = async (user: UserProfile) => {
      if (!onSyncProfile) {
          alert("Sync capability not available.");
          return;
      }
      
      if (!confirm(`Create a customer record for ${user.name}? This will allow them to book parcels.`)) return;
                ...
                {displayedUsers.map((user) => (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                            user.status === 'APPROVED' ? 'bg-green-50 text-green-700 border border-green-200' : 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 backdrop-blur-sm p-4">
              <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 animate-fade-in-up">
                  ...
                  <p className="text-center text-gray-600 mb-6 text-sm">
                      Are you sure you want to {confirmAction.type === 'APPROVED' ? 'approve' : 'reject'} <strong>{confirmAction.name}</strong>?
                      {confirmAction.type === 'APPROVED' ? ' They will gain access to the dashboard immediately.' : ' Their access will be blocked.'}
                  </p>
                  <div className="flex space-x-3">
                      ...
                      <Button 
                          ...
                          className={`w-full justify-center ${confirmAction.type === 'APPROVED' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select
                            className={`block w-full px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 sm:text-sm font-bold ${
                                selectedStatus === 'APPROVED' ? 'text-green-700 border-green-200 bg-green-50' : 'text-red-700 border-red-200 bg-red-50'
                            }`}
