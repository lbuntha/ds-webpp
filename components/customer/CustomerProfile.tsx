...
export const CustomerProfile: React.FC<Props> = ({ user }) => {
  ...
  const handleSaveRate = async () => {
      ...
      try {
          ...
          // @ts-ignore
          alert(t('rate_updated_success'));
      } catch (e) {
          console.error(e);
          // @ts-ignore
          alert(t('rate_updated_error'));
      } finally {
  ...
  const handleAddLocation = async () => {
      if (!newLocLabel || !newLocAddress) return alert("Label and Address are required.");
  ...
  const handleCreateBillingProfile = async () => {
      ...
      try {
          ...
          console.error(e);
          alert("Failed to initialize wallet profile.");
      } finally {
  ...
  const handleAddBank = async () => {
      if (!customerData) return;
      if (!newBankName || !newBankAccount) return alert("Bank Name and Account Number are required.");

      // Limit Check
      if ((customerData.bankAccounts?.length || 0) >= 2) {
          return alert(t('max_bank_limit'));
      }
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-20">
                ...
                <div className="space-y-4">
                    {customerData.bankAccounts && customerData.bankAccounts.map((bank, idx) => (
                        <div key={idx} className="border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-blue-200 transition-colors bg-white">
                            ...
                            <button onClick={() => handleDeleteBank(idx)} className="text-xs text-red-600 hover:underline font-medium px-2 py-1 rounded hover:bg-red-50">{t('remove')}</button>
                        </div>
                    ))}
                ...
                {locations.map(loc => (
                    <div key={loc.id} className="border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-red-200 transition-colors bg-white">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => handleDeleteLocation(loc.id)} className="text-xs text-red-600 hover:underline">{t('delete')}</button>
                        </div>
                    </div>
                ))}
        ...
        <Card title={t('referral_program')}>
            <div className="bg-red-50 p-6 rounded-xl text-center border border-red-100">
                ...
                <button 
                    onClick={() => { navigator.clipboard.writeText(displayReferralCode || ''); alert(t('copied')); }}
