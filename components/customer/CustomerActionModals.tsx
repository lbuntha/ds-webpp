...
export const CancelBookingModal: React.FC<CancelModalProps> = ({ isOpen, bookingId, onConfirm, onCancel, isLoading }) => {
    const { t } = useLanguage();
    if (!isOpen) return null;

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 backdrop-blur-sm p-4" onClick={isLoading ? undefined : onCancel}>
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex space-x-3">
                    <Button variant="outline" onClick={onCancel} className="w-full justify-center" disabled={isLoading}>
                        No, Keep
                    </Button>
                    <Button
                        onClick={onConfirm}
                        className="w-full justify-center bg-red-600 hover:bg-red-700 text-white flex items-center gap-2"
                        disabled={isLoading}
                    >
