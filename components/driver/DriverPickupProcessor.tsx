...
import { ParcelBooking, ParcelItem, UserProfile, Place } from '../../types';
...
export const DriverPickupProcessor: React.FC<Props> = ({ job, user, onSave, onFinish, onCancel }) => {
  const { t } = useLanguage();
  ...
  const handleSaveAndNext = async () => {
      ...
      if (!activeItem.receiverName || activeItem.receiverName.trim() === '' || activeItem.receiverName === 'Details in Photo') {
          alert("Please enter the Receiver Name from the image.");
          return;
      }
