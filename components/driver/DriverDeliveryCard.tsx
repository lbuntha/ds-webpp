...
export const DriverDeliveryCard: React.FC<Props> = ({ job, onZoomImage, onAction, onUpdateCod, onChatClick, hasBranches }) => {
  const { t } = useLanguage();
  ...
  return (
    <div className="space-y-3">
      {items.filter(i => i.status === 'PICKED_UP' || (i.status === 'IN_TRANSIT' && !i.targetBranchId)).map((item) => {
