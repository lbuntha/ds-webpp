...
export const DriverDashboard: React.FC<Props> = ({ user }) => {
  ...
  const handleSettlement = async () => {
      ...
      if (!settlementCalc.isBalanced) {
          alert("Total provided amount must match the total COD collected.");
          return;
      }

      // Ensure Banks are Configured
      if (settlementCalc.inputUSD > 0 && !defaultBankUSD) {
          return alert("System Error: No default USD settlement bank configured.");
      }
      if (settlementCalc.inputKHR > 0 && !defaultBankKHR) {
          return alert("System Error: No default KHR settlement bank configured.");
      }
      ...
      try {
          ...
          if (requestsToMake.length > 0) {
              ...
              alert("Settlement requested successfully. " + (requestsToMake.length > 1 ? "(Split into 2 requests)" : ""));
              setIsSettling(false);
              setSettleProof('');
              setPayAmountUSD('');
              setPayAmountKHR('');
      ...
      } catch (e) { alert("Failed to submit settlement request."); } finally { setIsSaving(false); }
  };
