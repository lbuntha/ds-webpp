...
export const PlaceManagement: React.FC = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            resetForm();
            await loadPlaces();
            toast.success(editingId ? "Place updated." : "Place saved.");
        } catch (e) {
    ...
    const handleSeed = async () => {
        ...
        try {
            ...
            await loadPlaces();
            toast.success("Sample places added! search 'Royal', 'Aeon' etc.");
        } catch (e) {
            console.error(e);
                    ...
                    <Card title={editingId ? "Edit Place" : "Add New Place"}>
                        ...
                        <div className="mt-6 pt-4 border-t border-gray-100 text-center">
                            ...
                            {!showSeedConfirm ? (
                                ...
                                <div className="flex flex-col gap-2 bg-yellow-50 p-2 rounded border border-yellow-200">
                                    <p className="text-xs text-yellow-800">Add sample places?</p>
                                    <div className="flex justify-center gap-2">
                                        <button onClick={() => setShowSeedConfirm(false)} className="text-xs text-gray-500 hover:underline">Cancel</button>
                                        <button onClick={handleSeed} className="text-xs bg-indigo-600 text-white px-2 py-1 rounded">Confirm</button>
                                    </div>
                                </div>
                            )}
                        </div>
                                            ...
                                            <td className="px-4 py-3 text-right text-sm font-medium space-x-2">
                                                ...
                                                {confirmDeleteId === p.id ? (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <button onClick={() => executeDelete(p.id)} className="text-xs bg-red-600 text-white px-2 py-1 rounded hover:bg-red-700">Yes</button>
                                                        <button onClick={() => setConfirmDeleteId(null)} className="text-xs text-gray-500 hover:text-gray-700">No</button>
                                                    </div>
                                                ) : (
                                                    <button onClick={() => handleDeleteClick(p.id)} className="text-red-600 hover:text-red-900">Delete</button>
                                                )}
                                            </td>
                                        </tr>
                                    ))}
